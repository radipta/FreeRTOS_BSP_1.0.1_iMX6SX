...
/*
 * Fungsi-fungsi Semaphore dan Queue
 */
uint32_t checkSemTCB()
{
	if(pxCurrentTCB->semPosition!=NULL)
	{
		return 1;
	}
...
uint32_t insertSemTCB(void* addr)
{
	pxCurrentTCB->semPosition=addr;
	//PRINTF("Address semp %x\n",pxCurrentTCB->semPosition);
	return 1;
}
void* getSemTCB()
{
	//PRINTF("thread addr %x",pxCurrentTCB);
	if(pxCurrentTCB->semPosition != NULL)
		return pxCurrentTCB->semPosition;
	return 0;
}
...
ListItem_t* getCurrentListTCB()
{
	//PRINTF(" List item %x\n",&(pxCurrentTCB->xEventListItem));
	return &(pxCurrentTCB->xEventListItem);
}
void delaytask(uint32_t timewake)
{
	//vListInsert( List, &( pxCurrentTCB->xEventListItem));
	const BaseType_t BlockIndefinitely=pdTRUE;
	uint32_t temp=xTickCount;
	if( uxListRemove( &( pxCurrentTCB->xGenericListItem ) ) == ( UBaseType_t ) 0 )
	{
		portRESET_READY_PRIORITY( pxCurrentTCB->uxPriority, uxTopReadyPriority );
	}

	if( ( timewake == portMAX_DELAY ) && ( BlockIndefinitely != pdFALSE ) )
	{
		vListInsertEnd( &xSuspendedTaskList, &( pxCurrentTCB->xGenericListItem ) );
	}
	else
	{
		listSET_LIST_ITEM_VALUE( &( pxCurrentTCB->xGenericListItem ), timewake );
		if( timewake < temp )
		{
		...
		else
		{
			...
			if( timewake < xNextTaskUnblockTime )
			{
				xNextTaskUnblockTime =timewake;
			}
		}
	}
}
