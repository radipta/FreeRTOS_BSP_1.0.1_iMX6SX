...
{
  //vTaskStartScheduler_Hamza(); // no idle task, gunakan ini saat multitasking
  vTaskStartScheduler(); // with idle task, gunakan ini saat hanya ada 1 task berjalan
  return osOK;
}
...
osThreadId osThreadPeriodicCreate (const osThreadDef_t *thread_def, void *argument, TickType_t period)
{
	TaskHandle_t handle;

	xTaskPeriodicCreate((TaskFunction_t)thread_def->pthread,(const portCHAR *)thread_def->name,
	              thread_def->stacksize, argument, makeFreeRtosPriority(thread_def->tpriority),
	              &handle,period);
...
osPoolId osPoolCreate (const osPoolDef_t *pool_def)
{
  ...
  thePool = Malloc_TLSF(sizeof(os_pool_cb_t),NONE,NONE);
  if (thePool) {
    ...
    thePool->markers = Malloc_TLSF(pool_def->pool_sz,NONE,NONE);
    if (thePool->markers) {
      /* Now allocate the pool itself. */
      //thePool->pool = pvPortMalloc(pool_def->pool_sz * itemSize);
    	thePool->pool = Malloc_TLSF(pool_def->pool_sz * itemSize,NONE,NONE);
      if (thePool->pool) {
